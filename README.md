...
키 매핑은 특정 키가 눌릴 때, 미리 설정한 기능을 호출하도록 한다. 약어 매크로와 크게 다를 건 없지만, 약어 매크로가 입력 모드, 명령행 모드에서만 사용가능한 반면, 키 매핑은 모든 모드에 대해 사용할 수 있다는 점이 다르다.

- 단축키 설정하기

명령어|설명
---|---
nmap key command|일반 모드에서 key를 누르면 command를 실행한다.
imap key command|입력 모드에서 key를 누르면 command를 실행한다.
vmap key command|비주얼 모드에서 key를 누르면 command를 실행한다.
cmap key command|명령행 모드에서 key를 누르면 command를 실행한다.

이 외에도 omap, xmap, smap, map!, lmap 등 다양한 단축키 매핑 명령이 있다. 단축키를 해제하는 명령은 map 대신 unmap을 사용하면 된다. 모드에 따라 각각 nunmap, iunmap, vunmap, cunmap 등으로 적으면 된다.

*caution*

단축키를 맵핑하기 전에는 반드시 해당 키에 다른 명령어가 맵핑되어 있지 않는지 확인하기 바란다. 특히 \<CTRL\>과 조합할 때는 미리 맵핑된 키가 많기 때문에 덮어 쓰지 않도록 주의!!

_이 밑으로 현재 필요없는 기능들인듯....._

> 자동 명령	

자동 명령은 특정 파일을 열거나 저장하는 이벤트가 발생할 때 사용자가 지정한 Vim 명령을 자동으로 수행시키는 기능이다. 이 기능을 이용하면 파일 형식에 따라 자동으로 특정 옵션을 켜거나 특정 명령을 내리는 것이 가능하다.
...
프로그래머라면 코드를 작성할 때 들여쓰기를 하게 된다. 들여쓰기가 코드의 가독성을 높여주기 때문이다.

*Note*

보안이나 다른 이유로 코드를 읽기 어렵게 만드는 난독화(obfuscation) 과정을 거치면 들여쓰기나 개행이 무시된다.

재정렬 기능은
```
=G
```
여기서 = 기호는 들여쓰기를 재정렬하는 명령이고, G는 재정렬을 적용할 범위를 나타낸다. %, }}, ]] 등을 사용할 수도 있다.

재정렬 명령은 오퍼레이션 펜딩 모드로 작동하므로 범위를 지정해야만 작동한다. 예를 들어 =100G라고 명령하면 현재 행부터 100번째 행까지만 들여쓰기가 재정렬 된다. 또한 비주얼 모드에서 재정렬을 적용할 범위를 선택한 후 = 키를 누르면, 선택한 범위에 대해서만 들여쓰기를 재정렬할 수 있다.

> 탭 대신 공백 사용하기

- 탭 대신 공백 입력

탭을 눌렀을 때 탭 대신 몇 개의 공백 문자를 넣는 :set expandtab 옵션이 제공된다. (줄여서 et라고 한다.) 예를 들어 설정 파일에 set et ts=4로 설정되어 있다면 사용자가 탭을 누르는 순간 4 칸의 공백 문자가 입력된다.

*Tip* *진짜 탭 문자를 입력하고 싶다면?*

expandtab 옵션이 설정된 상태에서 진짜 탭 문자를 입력하려면, 특수 문자를 입력할 때 썻던 \<CTRL-V\> 를 사용하면 된다. \<CTRL-V\> \<TAB\> 혹은 \<CTRL-V\> \<CTRL-I\> 로 입력하면 된다.

- 기존 탭 문자 \<-\> 공백 변환

그러나 et 옵션은 새로 입력하는 탭만 공백으로 바꿀 뿐 기존 탭 문자들은 건드리지 않는다. 따라서 기존의 탭 문자들도 모두 공백으로 변환하거나 반대로 공백을 탭으로 변환하려면 :retab 명령을 사용해야 한다. (줄여서 ret라고 한다.)

retab명령은 expandtab 옵션이 on 상태이면 탭을 공백으로 변환하고, off 상태면 공백을 탭으로 변환한다. ret 명령의 형태는 다음과 같다.
```
:[range]ret [new_ts]
```

여기서 new\_tx는 연속된 공백 문자의 길이이다. 만약 :ret 4라고 명령했다면 변환 대상은 공백 문자 네 칸이 된다. new\_ts 옵션을 생략하면 tabstop 설정을 따른다.

단, ret 명령을 사용하여 공백을 탭으로 변환할 때, 공백 문자로만 구성된 부분은 사용자가 의도적으로 입력했다고 판단해 변환하지 않는다. 이를 무시하고 모든 공백을 탭으로 변환하고 싶다면 강제 명령어인 느낌표(!)를 붙여 다음처럼 명령해야 한다.
```
:[range]ret! [new_ts]
```
...
프로그래밍을 하는 경우, 소스코드 좌측 부분의 들여쓰기에 대해서만 공백을 탭으로 변환하고 싶다면, ret 명령 대신 재정렬 기능(=)을 권장한다.

> 단어 완성

프로그래밍을 하다보면 변수나 함수 혹은 메서드 이름에서 오타가 발생하기도 한다. 이를 위해, 단어 완성(keyword completion) 기능이 도입되었다.

- 단어 자동 완성

단어 자동 완성은 본문에 이미 등장한 단어나 예약된 단어의 앞 글자 몇 개만 입력한 후 \<CTRL-N\>을 누르면 나머지를 완성시켜 주는 기능이다. 입력 모드에서 작동하기 때문에, 코드 작성 중 모드를 바꿀 필요 없이 바로바로 사용 할 수 있다.

hellomonkey.c 를 작성하고 나서 어느 부분에서나 mo까지만 입력한 뒤 \<CTRL-N\>을 누르면 monkey로 완성되는 것을 볼 수 있다. 
...
명령어|설명
---|---
\<CTRL-X\> \<CTRL-N\>|더하기 낱말 모드로 작동하여 추가 검색을 한다. 원하는 낱말이 아닌 경우에는 \<CTRL-N\>을 계속 누르면 된다.
\<CTRL-X\> \<CTRL-K\> \<CTRL-N\>|사전 검색 모드로 작동한다.

> HTML 변환하기

소스코드를 웹에 게재했을 때 들여쓰기나 줄바꿈 모양이 망가지는 경우가 있다. 

그래서 Vim에는 현재 화면에 보이는 상태를 HTML로 변환하는 기능이 탑재되어 있다. 이 기능을 사용하기 위해 앞서 연습했던 소스코드 파일을 하나 열고 :TOhtml 명령을 입력한다.

명령이 실행되면 창이 분할되면서 .html 확장자를 붙인 새로운 버퍼(파일)가 만들어 진다.

-----

### 플러그인

앞에서 이미 플러그인을 사용해 본 적이 있다. 디렉터리 탐색기인 netrw 플러그인, TOhtml을 살펴보았다. 이들은 Vim에 내장된 플러그인으로 별다른 설치 과정없이 사용할 수 있다. 하지만 이제부터 설명할 플러그인은 사용자가 수동으로 설치해야 한다.

*Note*

Vim은 스크립트 언어가 내장되어 있기 때문에, 사용자도 플러그인을 제작할 수 있다.

> 플러그인 설치

Vim 플러그인은 대부분 압축 파일로 제공된다. 따라서 사용자가 플러그인 파일을 특정 디렉터리에 넣어 주어야만 한다. 플러그인을 저장할 디렉터리 목록.
...
맥 OS X|~/.vim 디렉터리
윈도 계열|~/vimfiles 디렉터리
매킨토시|$VIM:vimfiles 디렉터리

Vim 웹사이트(http://www.vim.org)에 접속해 왼쪽 메뉴의 Scripts 항목에 들어가면 플러그인을 검색할 수 있다. 

ex) NERD tree(초보자도 쉽게 사용할 수 있는 직관적인 디렉터리 탐색기), tagList(소스코드를 분석해 변수, 매크로, 함수의 목록을 만들어준다.)
